...
Run the crowd-sniff command:

```bash
printing-press crowd-sniff --api <api> --output "$RESEARCH_DIR/<api>-crowd-spec.yaml"
```

If the API has a known base URL from Phase 1 research, pass it:

```bash
printing-press crowd-sniff --api <api> --base-url <known-base-url> --output "$RESEARCH_DIR/<api>-crowd-spec.yaml"
```
