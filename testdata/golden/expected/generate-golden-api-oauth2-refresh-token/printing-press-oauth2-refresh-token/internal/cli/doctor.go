...
import (
	...
	"printing-press-oauth2-refresh-token-pp-cli/internal/client"
	"printing-press-oauth2-refresh-token-pp-cli/internal/config"
	"printing-press-oauth2-refresh-token-pp-cli/internal/store"
...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  printing-press-oauth2-refresh-token-pp-cli doctor
  printing-press-oauth2-refresh-token-pp-cli doctor --json
  printing-press-oauth2-refresh-token-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				if cfg.ClientID != "" && cfg.ClientSecret != "" && cfg.RefreshToken != "" {
					if cfg.AccessToken == "" {
						report["auth"] = "configured (access-token cache empty)"
					} else if cfg.TokenExpiry.IsZero() || time.Now().Before(cfg.TokenExpiry) {
						report["auth"] = "configured"
					} else {
						report["auth"] = "configured (access-token cache expired)"
					}
					report["auth_source"] = "env"
				} else {
			...
			if os.Getenv("TEST_API_LWA_CLIENT_ID") != "" {
				authEnvSet = append(authEnvSet, "TEST_API_LWA_CLIENT_ID")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TEST_API_LWA_CLIENT_ID")
			}
			if os.Getenv("TEST_API_LWA_CLIENT_SECRET") != "" {
				authEnvSet = append(authEnvSet, "TEST_API_LWA_CLIENT_SECRET")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TEST_API_LWA_CLIENT_SECRET")
			}
			if os.Getenv("TEST_API_REFRESH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "TEST_API_REFRESH_TOKEN")
			} else {
				authEnvRequiredMissing = append(authEnvRequiredMissing, "TEST_API_REFRESH_TOKEN")
			}
			switch {
			...
			if cfg != nil && cfg.BaseURL != "" {
				...
				if clientErr != nil {
					...
					refreshTokenConfigured := cfg.ClientID != "" && cfg.ClientSecret != "" && cfg.RefreshToken != ""
					if authHeader == "" && !refreshTokenConfigured {
						...
						if authHeader != "" {
						authHeaders["x-api-access-token"] = authHeader
						}
						authHeaders["User-Agent"] = "printing-press-oauth2-refresh-token-pp-cli"
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	report := map[string]any{}
	dbPath := defaultDBPath("printing-press-oauth2-refresh-token-pp-cli")
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'printing-press-oauth2-refresh-token-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'printing-press-oauth2-refresh-token-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'printing-press-oauth2-refresh-token-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'printing-press-oauth2-refresh-token-pp-cli sync' to refresh."
	}
	return report
}
