...
func classifyAPIError(err error, flags *rootFlags) error {
	...
	case strings.Contains(msg, "HTTP 400") && cliutil.LooksLikeAuthError(msg):
		return authErr(fmt.Errorf("%w\nhint: the API rejected the request — this usually means auth is missing or invalid."+
			"\n      Run 'printing-press-oauth2-refresh-token-pp-cli doctor' to check auth status."+
			"\n      Response: "+cliutil.SanitizeErrorBody(msg), err))
	case strings.Contains(msg, "HTTP 401"):
		return authErr(fmt.Errorf("%w\nhint: check your OAuth client ID, client secret, refresh token, and granted scopes."+
			...
			"\n      Run 'printing-press-oauth2-refresh-token-pp-cli doctor' to check auth status.", err))
	case strings.Contains(msg, "HTTP 403"):
		return authErr(fmt.Errorf("%w\nhint: permission denied. Check that the refresh token grants the roles or scopes required for this resource."+
			"\n      Run 'printing-press-oauth2-refresh-token-pp-cli auth status' to check required environment variables and access-token cache."+
			"\n      Run 'printing-press-oauth2-refresh-token-pp-cli doctor' to check auth status.", err))
